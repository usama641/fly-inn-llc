...
export default function AddPolicy({ params }: { params: Promise<Params> }) {
  ...
  const { data: cancelPolicyData } = useApiGet({
    endpoint: `/stay/cancellation-policy/${editId}`,
    queryKey: ["cancellation-policy-s", editId],
    config: {
  ...
  useEffect(() => {
    if (cancelPolicyData) {
      reset({
        lodgingType: String(cancelPolicyData.type),
        featureTitle: cancelPolicyData.name,
        beforeCheckIn: cancelPolicyData.before_check_in,
        afterCheckIn: cancelPolicyData.after_check_in,
      });
    }
  }, [cancelPolicyData, reset]);
